...
import seq2seqDialog.word_token
import jieba
import random
...
wordToken = seq2seqDialog.word_token.WordToken()

# 放在全局的位置，为了动态算出num_encoder_symbols和num_decoder_symbols
max_token_id = wordToken.load_file_list(['./seq2seqDialog/samples/question.txt', './seq2seqDialog/samples/answer.txt'], min_freq)
...
def get_train_set():
    ...
    train_set = []
    with open('./seq2seqDialog/samples/question.txt', 'r',encoding='utf-8') as question_file:
        with open('./seq2seqDialog/samples/answer.txt', 'r',encoding='utf-8') as answer_file:
            while True:
    ...
    with tf.Session() as sess:
        ...
        for step in range(train_round):
            ...
            if step % 10 == 0:
                ...
                with open('./seq2seqDialog/train.log', 'a') as my_logger: 
                ...
                # 模型持久化
                saver.save(sess, './seq2seqDialog/model/dialogControl')
...
if STATE == "TEST":
    ...
    saver.restore(sess, './seq2seqDialog/model/dialogControl')
elif STATE == "TRAIN":
    pass
def dialog(input_seq):
